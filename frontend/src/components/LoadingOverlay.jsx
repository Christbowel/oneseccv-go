  ...
  return (
    <div className="animate-fade-in scanline-fx absolute inset-0 z-40 flex items-center justify-center px-4"
      ...
      <div className="relative w-full max-w-md overflow-hidden rounded-2xl p-6 sm:p-8"
        ...
        <h2 className="mb-0.5 text-center text-xl font-bold tracking-tight text-white" style={{ fontFamily: 'Ubuntu, sans-serif' }}>
          OneSecCV
        </h2>
        ...
        <div className="mb-2 flex min-h-[3rem] flex-col items-center justify-center text-center">
          ...
          <p className="mt-1 font-mono text-xs" style={{ color: '#9BA6BC' }}>{step.sub}</p>
        </div>

        <div className="mb-3 mt-6">
          <div className="mb-1.5 flex justify-between">
            <span className="font-mono text-xs" style={{ color: '#6C7488' }}>Progress</span>
          ...
          {STEPS.map((_, i) => (
            <div key={i} className="h-1 rounded-full transition-all duration-300"
              style={{
                ...
                background: i === stepIndex ? '#FF6B1A' : i < stepIndex ? 'rgba(255,107,26,0.3)' : '#272E40',
              }} />
          ))}
        </div>

        <p className="mt-5 text-center text-[11px]" style={{ color: '#6C7488' }}>
