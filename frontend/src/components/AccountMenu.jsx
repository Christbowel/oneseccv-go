  ...
  return (
    <div className="relative" ref={ref}>
      <button onClick={() => setOpen(o => !o)}
        ...
        style={{ border: '1.5px solid rgba(255,107,26,0.45)', background: '#0A0F1E' }}>
        {user.picture
      ...
      {open && (
        <div role="menu"
          ...
          style={{ background: '#080C18', border: '1px solid #1A2040', boxShadow: '0 18px 50px rgba(0,0,0,0.6)' }}>

          <div className="px-4 py-3" style={{ borderBottom: '1px solid #131A34' }}>
            <p className="truncate text-sm font-semibold text-white">{user.name}</p>
            <p className="truncate font-mono text-[11px]" style={{ color: '#7A83A0' }}>{user.email}</p>
          </div>

          <div className="px-4 py-3" style={{ borderBottom: '1px solid #131A34' }}>
            <p className="font-mono text-[10px] uppercase tracking-widest" style={{ color: '#7A83A0' }}>
              Storage
            </p>
            <p className="mt-1.5 text-xs leading-relaxed" style={{ color: '#A0A8C0' }}>
...
function MenuItem({ icon, label, onClick, danger }) {
  return (
    <button role="menuitem" onClick={onClick}
      ...
      style={{ color: danger ? '#EF6B6B' : '#C6CCE0' }}
